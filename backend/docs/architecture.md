...
flowchart LR
    ...
    ScenarioDB[(SQLite/PostgreSQL Scenario)]
    FacilityDB[(SQLite/PostgreSQL Facility)]
    Interface[swmm_engine.interface]
    Converter[React Layout -> SWMM INP Converter]
    Runtime[SwmmRuntimeEngine]
    PySWMM[PySWMM / EPA SWMM]
    Risk[Risk Detection]
    Logs[Runtime JSONL Logs]
    LLM[FastAPI + LangChain]
...
Dockerfile은 Python 3.12 slim 이미지를 사용한다. 컨테이너 시작 시
`python manage.py migrate`를 실행한 뒤 Daphne으로 `config.asgi:application`을
서비스한다.

Docker Compose는 SQLite 파일을 `sqlite_data` 볼륨의 `/data/db.sqlite3`에 보존한다.
PostgreSQL 설정은 `settings.py`에 준비되어 있지만 Compose에는 현재 PostgreSQL
서비스가 정의되어 있지 않다.
