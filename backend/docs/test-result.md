...
| 활성 auth/facilities/monitoring/notification/simulation 테스트 | 43개 통과 |
| 전체 테스트 발견 수                    | 43        |
| 전체 테스트 성공                       | 43        |
| 전체 테스트 실패                       | 0         |
| 전체 테스트 오류                       | 0         |
...
`apps.auth apps.facilities apps.monitoring apps.notification apps.simulation` 테스트는 43개 모두
통과했다.
...
현재 `manage.py test -v 2`의 전체 discovery 대상 43개 테스트는 모두 통과한다.
...
| facilities API                             |         3 | 통과 |
| monitoring 위험 로그 API, 예측, 저장 서비스, FastAPI maintenance 연동 |        15 | 통과 |
