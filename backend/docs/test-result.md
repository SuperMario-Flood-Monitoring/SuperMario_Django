...
`맑음`, `우천`, `호우`, `폭우`로 정규화하는 LEVEL 13 회귀 테스트와 LLM 발송
묶음/cooldown 정책을 검증하는 회귀 테스트, LLM 응답 timeout을
`dispatch_failed`로 분류하지 않는 회귀 테스트, 엔진 일시정지/정지 상태에서는
LLM POST를 건너뛰는 회귀 테스트, LLM POST 직전에 엔진을 자동 일시정지하고
React에 paused status를 전파하는 회귀 테스트가 포함된다. LEVEL 23 기준으로
일반 CRITICAL 위험은 aggregation window와 pending queue로 묶고, runtime
막힘/역류는 emergency aggregation window로 별도 처리하는 것도 같은 테스트에서
검증한다. 일반 위험은 유지시간을 채운 뒤에만 LLM trigger가 열리고, runtime
역류는 유지시간 대기 없이 trigger되는 lifecycle도 검증한다.

LEVEL 18 기준으로 `apps.monitoring`에는 현재 프로젝트의 실제 SWMM risk event
구조(`eventType`, `source`, `sourceId`, `severity`)를 기준으로 CRITICAL 위험
로그를 저장하는 테스트, event_key 중복 방지 테스트, REST 목록/상세/조치 저장
흐름 테스트가 포함된다.

LEVEL 19 기준으로 `apps.monitoring`에는 React에서 받은 장애 조치 원문을
FastAPI maintenance log endpoint로 전달하는 테스트, FastAPI 응답의 `vector_id`
저장 테스트, FastAPI 요청 실패 시에도 조치 저장과 완료 처리가 유지되고
`fastapi_sync_status=FAILED`로 기록되는 테스트가 포함된다. 현재 FastAPI 전송
body는 LEVEL 25 기준의 구조화 payload를 사용한다.
...
LEVEL 23 기준으로 LLM 문자 발송 정책을 aggregation window, cooldown window,
...
대상/조치 문자열 중심 형식에서 `event`, `metrics`, `action` 객체를 포함하는
구조화 payload로 변경했다. `HazardAction`에는
`result_detail`, `recurrence_note` 컬럼을 추가했다.

LEVEL 26 기준으로 위험 조치 흐름을 조치 시작과 조치 완료로 분리했다.
`POST /api/hazards/{id}/actions`는 조치 내용만 저장하고 위험 로그를
`IN_PROGRESS`로 변경하며 FastAPI 전송과 embedding 이력을 만들지 않는다.
`PATCH /api/hazards/{id}/actions/{action_id}`는 결과와 재발 참고사항을 저장하고
완료 시점에만 위험 로그를 `RESOLVED` 및 논리 삭제 처리한 뒤 FastAPI 전송과
...
| `apps.auth.tests.EnsureAdminUserCommandTests.test_updates_default_admin_even_when_other_admin_exists`               | 통과 |
| `apps.auth.tests.EnsureAdminUserCommandTests.test_updates_existing_default_admin`                                   | 통과 |
...
| `apps.monitoring.tests.HazardApiTests.test_action_completion_requires_result_detail`                             | 통과 |
| `apps.monitoring.tests.HazardApiTests.test_action_completion_resolves_event_and_dispatches_embedding_payload`    | 통과 |
...
| `apps.monitoring.tests.MaintenanceDispatcherTests.test_builds_fastapi_payload_with_prompt_level_25_shape`         | 통과 |
...
| `apps.simulation.tests.LangChainDispatchPayloadTests.test_queues_dispatch_during_aggregation_window`               | 통과 |
| `apps.simulation.tests.LangChainDispatchPayloadTests.test_queues_pending_during_cooldown_and_flushes_after_window` | 통과 |
| `apps.simulation.tests.LangChainDispatchPayloadTests.test_queues_runtime_blockage_in_emergency_window_without_cooldown` | 통과 |
| `apps.simulation.tests.LangChainDispatchPayloadTests.test_pause_engine_for_llm_dispatch_broadcasts_paused_status` | 통과 |
| `apps.simulation.tests.LangChainDispatchPayloadTests.test_pauses_engine_and_broadcasts_realtime_alert_right_before_llm_post` | 통과 |
| `apps.simulation.tests.LangChainDispatchPayloadTests.test_response_timeout_is_not_classified_as_dispatch_failed`    | 통과 |
| `apps.simulation.tests.LangChainDispatchPayloadTests.test_skips_llm_post_when_engine_is_paused_or_stopped`         | 통과 |
...
| `apps.simulation.tests.RiskLifecycleTriggerTests.test_runtime_reverse_flow_triggers_without_sustain_delay`         | 통과 |
